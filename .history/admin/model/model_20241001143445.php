...
class UserModel {
   ...
   // contact 
        ...
        public function getAllItems() {
            return $this->conn->query("SELECT * FROM shopping_items ORDER BY id DESC");
        }
    
        public function addItem($imageUrl, $title, $description, $price, $status) {
            $stmt = $this->conn->prepare("INSERT INTO shopping_items (image_url, title, description, price, status) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssis", $imageUrl, $title, $description, $price, $status);
            return $stmt->execute();
        }
    
        public function editItem($title, $description, $price, $id) {
