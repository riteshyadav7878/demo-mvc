...
class UserModel {
   ...
   // contact 
    ...
    public function getAllshop($showActiveOnly = true) {
        $query = "SELECT * FROM shopping_items" . ($showActiveOnly ? " WHERE status = 'active'" : "") . " ORDER BY id DESC";
        return $this->conn->query($query)->fetch_all(MYSQLI_ASSOC);
    }
    

    public function getAllshops($showActiveOnly = true) {
        $query = "SELECT * FROM shopping_items" . ($showActiveOnly ? " WHERE status = 'active'" : "") . " ORDER BY id DESC LIMIT 8";
        $result = $this->conn->query($query);
        return $result->fetch_all(MYSQLI_ASSOC);
    }
    ...
    public function getItemById($itemId) {
        ...
        return $result->fetch_assoc(); // Returns the item details as an associative array
    }
