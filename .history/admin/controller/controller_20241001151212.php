...
class UserController {
    ...
    public function store() {
        $messages = [];
        $items = $this->getItems(); // Fetch items using the corrected method

        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if (isset($_FILES['itemImage'])) {
                $messages = array_merge($messages, $this->userModel->addItem($_POST, $_FILES['itemImage']));
            } elseif (isset($_POST['editId'])) {
                $messages = array_merge($messages, $this->userModel->editItem($_POST['editId'], $_POST));
            } elseif (isset($_POST['deleteId'])) {
                $messages = array_merge($messages, $this->userModel->deleteItem($_POST['deleteId']));
            } elseif (isset($_POST['toggleStatusId'])) {
                $messages = array_merge($messages, $this->userModel->toggleItemStatus($_POST['toggleStatusId']));
            }
        }
    ...
    public function getItems() {
        $query = "SELECT * FROM items"; // Adjust this according to your database structure
        return $this->conn->query($query); // Use the initialized connection here
    }
}
?>
