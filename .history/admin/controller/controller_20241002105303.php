...
class UserController {
    ...
    public function handlestore() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if (isset($_FILES['itemImage'])) {
                ...
                $messages[] = $this->userModel->editItem($_POST['editId'], $_POST['itemTitle'], $_POST['itemDescription'], $_POST['itemPrice']) ? "Record updated successfully!" : "Error updating record.";
            } elseif (isset($_POST['deleteId'])) {
                $messages[] = $this->userModel->deleteItem($_POST['deleteId']) ? "Item deleted successfully!" : "Error deleting item.";
            } elseif (isset($_POST['toggleStatusId'])) {
                $messages[] = $this->userModel->toggleStatus($_POST['toggleStatusId']) ? "Status toggled successfully!" : "Error toggling status.";
            }
        }

        return [$this->userModel->getAllItems(), $messages];
    }
    ...
    private function handleImageUpload($messages) {
        ...
        if ($uploadOk === 0) {
            ...
            if (move_uploaded_file($_FILES["itemImage"]["tmp_name"], $target_file)) {
                $this->userModel->addItem($target_file, $_POST['itemTitle'], $_POST['itemDescription'], $_POST['itemPrice'], $_POST['itemStatus']);
    ...
    {
        return $this ->userModel->getAllItems();
    }

    public function fetchshop() 
    {
        return $this ->userModel->getAllshop();
    }
    
    public function inexshop() 
    {
        return $this ->userModel->getAllshops();
    }
   
    public function cart() 
    {
        return $this ->userModel->getItemById($itemId);
    }
