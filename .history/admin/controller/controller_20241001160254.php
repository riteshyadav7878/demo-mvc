...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['itemImage'])) {
        handleImageUpload($model, $messages);
    } elseif (isset($_POST['editId'])) {
        handleEdit($model, $messages);
    } elseif (isset($_POST['deleteId'])) {
        handleDelete($model, $messages);
    } elseif (isset($_POST['toggleStatusId'])) {
        toggleStatus($model, $messages);
    }
}
...
function handleImageUpload($model, &$messages) {
    $uploadOk = 1;
    $target_dir = "uploads/";
    ...
    if ($uploadOk === 0) {
        ...
        if (move_uploaded_file($_FILES["itemImage"]["tmp_name"], $target_file)) {
            if ($model->addItem($target_file, $_POST['itemTitle'], $_POST['itemDescription'], $_POST['itemPrice'], $_POST['itemStatus'])) {
                $messages[] = "New Item Added Successfully!";
            } else {
                $messages[] = "Error adding item.";
            }
        } else {
...
function handleEdit($model, &$messages) {
    if ($model->editItem($_POST['editId'], $_POST['itemTitle'], $_POST['itemDescription'], $_POST['itemPrice'])) {
...
function handleDelete($model, &$messages) {
    if ($model->deleteItem($_POST['deleteId'])) {
        $messages[] = "Item deleted successfully!";
    } else {
        $messages[] = "Error deleting item.";
    }
}

// Function to toggle item status
function toggleStatus($model, &$messages) {
    if ($model->toggleItemStatus($_POST['toggleStatusId'])) {
