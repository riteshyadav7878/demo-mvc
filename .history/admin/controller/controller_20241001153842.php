...
class UserController {
    ...
    public function getItems() {
        $sql = "SELECT * FROM items"; // Adjust the table name if needed
        return $this->conn->query($sql);
    }

    public function addItem($title, $description, $price, $status, $image) {
        $stmt = $this->conn->prepare("INSERT INTO items (title, description, price, status, image_url) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("ssdss", $title, $description, $price, $status, $image);
        $stmt->execute();
        $stmt->close();
    }
    ...
    public function toggleItemStatus($id, $status) {
        ...
        $stmt = $this->conn->prepare("UPDATE items SET status = ? WHERE id = ?");
