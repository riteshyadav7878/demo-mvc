...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['editId']) && isset($_POST['itemTitle'])) {
    $stmt = $conn->prepare("UPDATE shopping_items SET title=?, description=?, price=?, status=? WHERE id=?");
    $stmt->bind_param("ssdsi", $_POST['itemTitle'], $_POST['itemDescription'], $_POST['itemPrice'], $_POST['itemStatus'], $_POST['editId']);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['toggleStatusId'])) {
    $stmt = $conn->prepare("SELECT status FROM shopping_items WHERE id=?");
    ...
    $stmt->bind_param("si", $newStatus, $_POST['toggleStatusId']);
    
    if ($stmt->execute()) {
        echo "<div class='alert alert-success'>Item status updated to $newStatus!</div>";
    } else {
        echo "<div class='alert alert-danger'>Error: " . $stmt->error . "</div>";
    }
    $stmt->close();
                    ...
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-warning btn-sm" 
                                    ...
                                    onclick="populateEditModal(<?php echo json_encode($row); ?>)">
                                Edit
                            </button>
                ...
                <div class="modal-body">
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="editItemStatus" name="itemStatus" required>
                                <option value="Active">Active</option>
                                <option value="Inactive">Inactive</option>
                            </select>
                        </div>
